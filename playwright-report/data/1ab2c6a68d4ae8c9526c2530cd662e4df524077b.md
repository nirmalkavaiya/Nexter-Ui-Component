...
- Name: keyboard-navigation.spec.js >> Tabs — keyboard >> ArrowRight moves focus to next tab
- Location: e2e\keyboard-navigation.spec.js:74:7
  ...
  - generic [ref=e267]:
    ...
    - generic [ref=e272]:
      - generic [ref=e273]:
        ...
        - tablist [ref=e275]:
          - generic [ref=e276]:
            - tab "General" [active] [selected] [ref=e277] [cursor=pointer]
...
> 83  |     expect(focused).not.toBe(firstText);
  ...
  168 |     await openBtn.scrollIntoViewIfNeeded();
  169 |     await openBtn.click();
  170 |     await expect(page.locator('[role="dialog"]').first()).toBeVisible();
  171 |     await page.keyboard.press('Escape');
  172 |     await expect(page.locator('[role="dialog"]').first()).not.toBeVisible();
  173 |   });
  174 | 
  175 |   test('focus moves into modal on open', async ({ page }) => {
  176 |     const openBtn = page.getByRole('button', { name: /open modal/i }).first();
  177 |     await openBtn.scrollIntoViewIfNeeded();
  178 |     await openBtn.click();
  179 |     const dialog = page.locator('[role="dialog"]').first();
  180 |     await expect(dialog).toBeVisible();
  181 |     // Wait for focus-trap; allow up to 500ms for focus to settle
  182 |     await page.waitForTimeout(500);
  183 |     const activeInsideModal = await page.evaluate(() => {
```